...
export default function FacilityMapInner({ facilities }: FacilityMapProps) {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {/* Legend */}
      <div className="absolute top-3 left-3 z-[400] bg-white/90 backdrop-blur-sm rounded-xl px-3 py-2 shadow-card border border-neutral-150 flex flex-col gap-1.5">
        {(['hospital', 'clinic', 'pharmacy'] as const).map((type) => (
          <div key={type} className="flex items-center gap-2">
            ...
            />
            <span className="text-[11px] font-semibold text-neutral-600 capitalize">{type}</span>
          </div>
        ))}
      </div>
          ...
          >
            <div className="relative">
              <button
                ...
                className="absolute -top-2 -right-2 z-10 w-6 h-6 bg-white border border-neutral-200 rounded-full shadow-card flex items-center justify-center"
                aria-label="Close"
              >
